...
class FlightTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.flight = Flight.objects.create(flight_number='1234ABC',
    ...
    def test_flights_create_as_auth_user(self):
        request = self.factory.post(
            '/flight/flights/', HTTP_AUTHORIZATION=f'Token {self.token}')
        response = FlightView.as_view({'post': 'create'})(request)
        self.assertEqual(response.status_code, 403)

    def test_flights_create_as_staff_user(self):
        data = {
            "flight_number": '1234ABC',
            "operation_airlines": 'THY',
            "departure_city": 'Adana',
            "arrival_city": 'Ankara',
